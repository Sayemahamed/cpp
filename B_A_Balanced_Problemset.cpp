...
// solve function//
void solve() {
    long long problem, parts, minDivider;cin >> problem >> parts;
    vector<long long >factors;
    minDivider = problem;
    for (auto& it : primeFactors( problem )) {
        while (it.second--)factors.push_back( it.first );
    }
    for (int i = 0;i < factors.size();i++) {
        int j = i;
        long long temp = 1;
        while (j < factors.size() and temp < parts) {
            temp *= factors[ j++ ];
        }
        if (temp >= parts)minDivider = min( minDivider, temp );
    }
    cout << problem / minDivider << endl;
}
