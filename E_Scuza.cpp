...
void solve()
{
    long long stairs,qur;cin>>stairs>>qur;
    vector<long long>dat(stairs),siz(stairs,0);
    ...
    for(long long i=0;i<stairs;i++)
    {
        if(ans.empty())ans[dat[i]]=siz[i];
        if(dat[i]>=ans.rbegin()->first)
        {
            ans[dat[i]]=siz[i];
        }
        else
        ans.rbegin()->second=siz[i];
    }
    while(qur--)
    {
        long long x;cin>>x;
